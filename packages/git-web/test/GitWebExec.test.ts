...
test('exec with unknown command throws error', async () => {
  ...
  await expect(exec('git', ['unknown-command'], { cwd: 'web://test' })).rejects.toThrow(
    "git: 'unknown-command' is not a git command. See 'git --help'.",
  )
...
test('exec with empty args throws error', async () => {
  ...
  await expect(exec('git', [], { cwd: 'web://test' })).rejects.toThrow('usage: git')
